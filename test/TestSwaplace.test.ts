...
describe("Swaplace", async function () {
  ...
  it("Should not be able to cancel not owned swaps", async function () {
    ...
    await expect(Swaplace.cancelSwap(Number(lastSwap))).to.be.revertedWithCustomError(
      Swaplace,
  ...
  it("Should revert when accepting swaps that were already accepted", async function () {
    ...
    expect(await Swaplace.connect(acceptee).acceptSwap(Number(lastSwap))).to.be.ok;
  ...
  it("Should revert when accepting swaps with lacking allowance { ERC20 }", async function () {
    ...
    await expect(Swaplace.connect(acceptee).acceptSwap(Number(lastSwap))).to.be.revertedWith(
  ...
  it("Should be able to accept a swap with only { ERC20 }", async function () {
    ...
    const assetTypes = [0, 0]; // 0 = ERC20, 1 = ERC721
    ...
    expect(await MockERC20.allowance(owner.address, Swaplace.address)).to.be.equal("1000");
    expect(await MockERC20.allowance(acceptee.address, Swaplace.address)).to.be.equal("10000");
  ...
  it("Should be able to accept a swap with only { ERC721 }", async function () {
    ...
    const assetTypes = [1, 1]; // 0 = ERC20, 1 = ERC721
    ...
    expect(await MockERC721.getApproved(tokenId - 1)).to.be.equal(Swaplace.address);
    expect(await MockERC721.getApproved(tokenId)).to.be.equal(Swaplace.address);
  ...
  it("Should be able to accept a swap N-1", async function () {
    ...
    const assetTypes = [0, 1, 1, 1]; // 0 = ERC20, 1 = ERC721
    ...
    // Expect the acceptee to have the token
    expect(await MockERC20.balanceOf(acceptee.address)).to.be.equal(balanceBefore.add(1000));
  });
  ...
  it("Should be able to accept a swap N-N", async function () {
    ...
    const assetTypes = [0, 1, 0, 1]; // 0 = ERC20, 1 = ERC721
    ...
    expect(await MockERC20.balanceOf(owner.address)).to.be.equal(balanceBeforeOwner);
    expect(await MockERC20.balanceOf(acceptee.address)).to.be.equal(balanceBefore);
  });
});
